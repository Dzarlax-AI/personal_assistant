...
- `claude_bridge.go` — Codex Bridge provider. Sends prompts to `Codex-bridge` HTTP service on the host, which wraps `Codex -p` CLI. `buildPrompt(messages, systemPrompt)` flattens multi-turn history into `User: ... / Assistant: ...` text format (CLI doesn't support multi-turn). Multimodal parts replaced with `[image]`/`[audio]`/`[document]` placeholders. Returns `APIError` on bridge errors for fallback routing. No tool calling — Codex CLI handles MCP tools itself via `--project-dir`.
...
- Auth via generic `headers` map (Codex Desktop format)
...
| File | Purpose |
|---|---|
| `.env` | Secrets: `TELEGRAM_BOT_TOKEN`, `DEEPSEEK_API_KEY`, `GEMINI_API_KEY`, `QWEN_API_KEY`, `OLLAMA_API_KEY` (optional), `TELEGRAM_OWNER_CHAT_ID`, `TZ` (default `Europe/Belgrade`), `EMBED_API_KEY` (optional, HF-TEI), `CLAUDE_BRIDGE_TOKEN` (optional, shared secret with Codex-bridge) |
...
| `config/mcp.json` | MCP servers in Codex Desktop format |
...
| `templates/AGENTS.md` | yes | Deployed to `/assistant_context/AGENTS.md` — same built-ins-only scope as `.example` |
| `config/system_prompt.md` | no (`.gitignored`) | User's local working copy. Used as fallback system prompt when filesystem is disabled; otherwise `/assistant_context/AGENTS.md` is primary. |
...
Authoritative `cost_usd` comes from OpenRouter's `usage.cost` (after upstream routing) or Codex Bridge's `total_cost_usd` (from `Codex -p --output-format json`). Gemini/Ollama/local providers report tokens only; cost = 0 for them.
...
`bridge/` contains a standalone Go HTTP service that runs on the host and wraps `Codex -p` CLI. The bot (in Docker) calls it as any other LLM provider.

```
Bot (Docker) → POST /ask → Codex-bridge (host:9900) → Codex -p → response
```

- `bridge/main.go` — HTTP service: `/ask` (prompt → CLI → response), `/health`. Zero external dependencies (Go stdlib only)
- Config via env vars: `CLAUDE_BRIDGE_TOKEN`, `CLAUDE_BRIDGE_PROJECT_DIR`, `CLAUDE_BRIDGE_LISTEN` (default `127.0.0.1:9900`), `CLAUDE_BRIDGE_CLI` (default `Codex`), `CLAUDE_BRIDGE_TIMEOUT` (default `120`), `CLAUDE_BRIDGE_CONCURRENCY` (default `2`)
- Bearer auth on all endpoints; semaphore limits concurrent CLI calls
- `cmd.Dir` set to project dir — CLI reads AGENTS.md and .mcp.json from cwd
- JSON output parsing with raw text fallback; `APIError` for router fallback routing
- `scripts/init-context.sh` creates the project context directory with AGENTS.md, settings.json, and MCP config symlink
- `scripts/setup.sh --with-Codex /path` — full setup: context dir, build/download bridge binary, generate auth token

### Model config structure

`models:` in `config.yaml` is a **map** (`type ModelsConfig map[string]ModelConfig` in `internal/config/config.go`). Each entry's YAML key becomes its routing name (referenced from `routing.default`, `routing.complex`, etc.). The entry's `provider` field selects the backend: `openrouter`, `gemini`, `ollama`, `Codex-bridge`, `local` for LLMs; `hf-tei`, `openai` for embeddings. The special key `embedding` is reserved for the MCP embedding provider and is not registered as an LLM.
...
The deploy config provisions four OR-backed model slots (`simple-or`, `default-or`, `complex-or`, `compaction-or`), each with its own `model:` id. Routing roles point at them 1:1 (simple→simple-or, default→default-or, complex→Codex-bridge by default but can switch to complex-or, compaction→compaction-or). Admin UI reassigns each slot's model id independently without side effects on the others. A single shared OR slot (the old `workhorse` pattern) would change all three roles at once — that's why it was split.
...
**Styling primitives come from the dzarlax design system** (`/static/dzarlax.css`, downloaded at Docker build time pinned to a commit SHA via `ARG DS_VERSION` in the Dockerfile — do NOT use `@main`, jsdelivr caches it for hours). Components in use: `.navbar` + `.navbar__brand` for the sticky brand header; `.tab-nav` + `.tab-nav a.active` for page tabs; `.kpi-bar` + `.kpi-item` (+ `__label` / `__value` / `__note`) for the Analytics metric row; `.divider--labeled` + `.divider__label` for chat session breaks; `.card` / `.card__header` / `.card__body` for content panels. Base rules from DS: `[hidden]{display:none!important}` (fixes `hidden` attribute on `.btn`), `input/select/textarea{font-size:max(16px,1rem)}` (iOS no-zoom). `partials_layout.html` only defines admin-specific overrides (filters, preset-banner, usage charts, per-breakpoint mobile stacking for settings/mcp forms) — never duplicate DS components locally. To pick up DS changes, bump `DS_VERSION` in `Dockerfile` to the new commit SHA.
