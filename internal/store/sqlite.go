...
func (s *SQLite) UsageByDay(ctx context.Context, since time.Time) ([]llm.UsageDayBucket, error) {
	rows, err := s.db.QueryContext(ctx, `
	...
	for rows.Next() {
		...
		if err := rows.Scan(&dayStr, &b.Calls, &b.PromptTokens, &b.CompletionTokens, &b.CostUSD); err != nil {
			return nil, err
		}
		b.Day, _ = time.Parse("2006-01-02", dayStr)
		out = append(out, b)
	}
	return out, nil
}

func (s *SQLite) UsageByModel(ctx context.Context, since time.Time, limit int) ([]llm.UsageModelRow, error) {
	rows, err := s.db.QueryContext(ctx, `
...
func (s *SQLite) UsageByRole(ctx context.Context, since time.Time) ([]llm.UsageRoleRow, error) {
	rows, err := s.db.QueryContext(ctx, `
...
func (s *SQLite) ExpensiveTurns(ctx context.Context, since time.Time, limit int) ([]llm.ExpensiveTurn, error) {
	rows, err := s.db.QueryContext(ctx, `
	...
	for rows.Next() {
		...
		var tsStr string
		if err := rows.Scan(&tsStr, &t.ChatID, &t.Role, &t.ModelID, &t.CostUSD, &t.Tokens, &t.Question, &t.Answer); err != nil {
			return nil, err
		}
		t.Ts, _ = time.Parse("2006-01-02 15:04:05", tsStr)
		out = append(out, t)
	}
	return out, nil
}
